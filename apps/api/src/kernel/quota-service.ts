...
export interface Reservation {
  ...
  periodStart: Date;
  quantity: number;
  status?: 'pending' | 'committed' | 'released' | undefined;
}
...
export class QuotaService {
  ...
  async reserve(
    ...
    const periodStart = firstUpdated.period_start;
    ...
    if (!firstInserted) {
      throw new Error('failed_to_insert_quota_reservation');
    }

    return { id: firstInserted.id, tenantId, quotaKey, periodStart, quantity, status: 'pending' };
  }
